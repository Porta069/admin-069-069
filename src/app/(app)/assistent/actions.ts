...
/**
 * KI-Assistent nach der PORTAWERK-Token-Doktrin.
 *
...
async function betriebZusammenfassung(
  ...
  if (kiVerfuegbar()) {
    const res = await kiJson<{ zusammenfassung: string }>({
      ...
      stufe: "guenstig",
      system:
        "Du bist der interne Assistent von PORTAWERK (Vermittlung von Handwerkern an Betriebe). " +
...
async function emailEntwurf(
  ...
  const res = await kiJson<{ betreff: string; text: string }>({
    ...
    stufe: "guenstig",
    system:
      "Du entwirfst kurze, freundliche und professionelle E-Mails auf Deutsch im Namen des Vermittlungsteams von PORTAWERK (vermittelt Handwerker an Betriebe). " +
      "Sprich den Empfänger höflich mit Sie an. Halte dich knapp und konkret, keine Floskeln, kein Fließtext über 120 Wörter. " +
      "Gib einen prägnanten Betreff und den E-Mail-Text (mit Anrede und Grußformel 'Mit freundlichen Grüßen, Ihr PORTAWERK-Team').",
