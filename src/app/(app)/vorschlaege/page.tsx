import Link from "next/link";
import { requireEmployee, can } from "@/lib/auth";
import { sql } from "@/lib/db";
import {
  readTableParams,
  safeSort,
  firstParam,
  type SearchParams,
} from "@/lib/table-params";
import { formatDate, formatNumber } from "@/lib/format";
import { PageHeader } from "@/components/common/page-header";
import { KpiCard } from "@/components/common/kpi-card";
import { StatusBadge } from "@/components/common/status-badge";
import { EmployeeAvatar } from "@/components/common/employee-avatar";
import {
  DataTable,
  type DataTableColumn,
  type DataTableRow,
} from "@/components/data-table/data-table";
import { FilterSelect } from "@/components/data-table/filter-select";
import { cn } from "@/lib/utils";
import { PROPOSAL_STATUS } from "./_components/status";
import { CreateProposalDialog } from "./_components/create-proposal-dialog";
import { ProposalActions } from "./_components/proposal-actions";

const COLUMNS: DataTableColumn[] = [
  { key: "kandidat", label: "Kandidat", sortable: true },
  { key: "stelle", label: "Stelle" },
  { key: "unternehmen", label: "Unternehmen" },
  { key: "score", label: "Match", sortable: true, className: "text-right" },
  { key: "status", label: "Status" },
  { key: "mitarbeiter", label: "Mitarbeiter" },
  { key: "erstellt", label: "Erstellt", sortable: true },
  { key: "aktion", label: "" },
];

/** Farbcode für den Match-Score analog zum Radar. */
function scoreClass(score: number | null): string {
  if (score === null) return "text-muted-foreground";
  if (score >= 80) return "text-success";
  if (score >= 60) return "text-warning";
  return "text-destructive";
}

export default async function ProposalsPage({
  searchParams,
}: {
  searchParams: Promise<SearchParams>;
}) {
  const employee = await requireEmployee("placements");
  const params = await searchParams;
  const { page, pageSize, q, sort, dir } = readTableParams(params, {
    sort: "erstellt",
  });
  const status = firstParam(params.status);

  const canCreate = can(employee, "placements", "create");
  const canEdit = can(employee, "placements", "edit");

  const orderBy = safeSort(
    sort,
    {
      kandidat: "candidate_name",
      score: "match_score",
      erstellt: "created_at",
    },
    "created_at",
  );
  const offset = (page - 1) * pageSize;
  const like = `%${q}%`;

  const [rows, [{ count }], statusCounts, pricingRows] = await Promise.all([
    sql`
      select p.*, e.name as employee_name, e.avatar_color
      from admin.proposal p
      left join admin.employee e on e.id = p.employee_id
      where p.deleted_at is null
        ${
          q
            ? sql`and (p.candidate_name ilike ${like} or p.company_name ilike ${like} or coalesce(p.job_title, '') ilike ${like})`
            : sql``
        }
        ${status ? sql`and p.status = ${status}` : sql``}
      order by
        case when p.status in ('VERMITTELT','ABGELEHNT') then 1 else 0 end asc,
        ${sql.unsafe(orderBy)} ${dir === "asc" ? sql`asc` : sql`desc`} nulls last
      limit ${pageSize} offset ${offset}`,
    sql`
      select count(*)::int as count
      from admin.proposal p
      where p.deleted_at is null
        ${
          q
            ? sql`and (p.candidate_name ilike ${like} or p.company_name ilike ${like} or coalesce(p.job_title, '') ilike ${like})`
            : sql``
        }
        ${status ? sql`and p.status = ${status}` : sql``}`,
    sql`
      select status, count(*)::int as count
      from admin.proposal
      where deleted_at is null
      group by status`,
    sql`select value from admin.setting where key = 'pricing'`,
  ]);

  const pricing = (pricingRows[0]?.value ?? {}) as Record<string, unknown>;
  const baseFeeCents =
    typeof pricing.base_fee_cents === "number" ? pricing.base_fee_cents : 4900;
  const maxCommissionCents =
    typeof pricing.max_commission_cents === "number"
      ? pricing.max_commission_cents
      : 250000;

  const byStatus: Record<string, number> = {};
  let totalProposals = 0;
  for (const r of statusCounts) {
    byStatus[r.status as string] = r.count as number;
    totalProposals += r.count as number;
  }
  const offen =
    (byStatus.VORGESCHLAGEN ?? 0) + (byStatus.BETRIEB_INTERESSIERT ?? 0);
  const imAngebot = (byStatus.ANGEBOT ?? 0) + (byStatus.ANGENOMMEN ?? 0);
  const vermittelt = byStatus.VERMITTELT ?? 0;
  const quote =
    totalProposals > 0
      ? Math.round((vermittelt / totalProposals) * 100)
      : 0;

  // Auswahllisten nur laden, wenn der Dialog überhaupt gezeigt wird.
  const [candidateRows, jobRows] = canCreate
    ? await Promise.all([
        sql`select id, "firstName" || ' ' || "lastName" as name, profession, phone
            from admin.candidate
            where status <> 'ERASED'
            order by "createdAt" desc limit 300`,
        sql`select j.id, j.title, c.name as company_name
            from public."JobPosting" j
            left join public."Company" c on c.id = j."companyId"
            where j.status <> 'ARCHIVED'
            order by j."createdAt" desc limit 500`,
      ])
    : [[], []];

  const tableRows: DataTableRow[] = rows.map((r) => {
    const scoreVal = r.match_score === null ? null : Number(r.match_score);
    return {
      id: r.id as string,
      cells: {
        kandidat: r.application_id ? (
          <Link
            href={`/kandidaten/${r.application_id}`}
            className="font-medium hover:underline"
          >
            {r.candidate_name as string}
          </Link>
        ) : (
          <span className="font-medium">{r.candidate_name as string}</span>
        ),
        stelle: r.job_posting_id ? (
          <Link href={`/stellen/${r.job_posting_id}`} className="hover:underline">
            {(r.job_title as string | null) ?? "—"}
          </Link>
        ) : (
          ((r.job_title as string | null) ?? "—")
        ),
        unternehmen: r.company_id ? (
          <Link href={`/unternehmen/${r.company_id}`} className="hover:underline">
            {(r.company_name as string | null) ?? "—"}
          </Link>
        ) : (
          ((r.company_name as string | null) ?? "—")
        ),
        score: (
          <span
            className={cn(
              "font-display text-base font-semibold tabular",
              scoreClass(scoreVal),
            )}
          >
            {scoreVal === null ? "—" : `${scoreVal} %`}
          </span>
        ),
        status: (
          <StatusBadge map={PROPOSAL_STATUS} value={r.status as string} />
        ),
        mitarbeiter: r.employee_name ? (
          <span className="inline-flex items-center gap-2">
            <EmployeeAvatar
              name={r.employee_name as string}
              color={r.avatar_color as string | null}
              size="sm"
            />
            <span className="text-sm">{r.employee_name as string}</span>
          </span>
        ) : (
          "—"
        ),
        erstellt: (
          <span className="tabular">{formatDate(r.created_at as string)}</span>
        ),
        aktion: canEdit ? (
          <ProposalActions
            id={r.id as string}
            status={r.status as string}
            candidateName={r.candidate_name as string}
            jobTitle={r.job_title as string | null}
            companyName={r.company_name as string | null}
            baseFeeCents={baseFeeCents}
            maxCommissionCents={maxCommissionCents}
          />
        ) : null,
      },
    };
  });

  return (
    <>
      <PageHeader
        title="Vorschläge & Angebote"
        description="Vom Vorschlag über das Angebot bis zur Vermittlung — der komplette Angebots-Workflow."
        actions={
          canCreate ? (
            <CreateProposalDialog
              candidates={candidateRows.map((c) => ({
                value: c.id as string,
                label: c.name as string,
                hint: [c.profession, c.phone].filter(Boolean).join(" · ") || undefined,
              }))}
              jobs={jobRows.map((j) => ({
                value: j.id as string,
                label: j.title as string,
                hint: (j.company_name as string | null) ?? undefined,
              }))}
            />
          ) : null
        }
      />

      <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiCard
          label="Offene Vorschläge"
          value={formatNumber(offen)}
          hint="Vorgeschlagen + interessiert"
        />
        <KpiCard
          label="Im Angebot"
          value={formatNumber(imAngebot)}
          hint="Angebot + angenommen"
          accent
        />
        <KpiCard
          label="Vermittelt"
          value={formatNumber(vermittelt)}
          hint="erfolgreich abgeschlossen"
        />
        <KpiCard
          label="Vermittlungsquote"
          value={`${quote} %`}
          hint="vermittelt / alle Vorschläge"
        />
      </div>

      <DataTable
        tableId="vorschlaege"
        columns={COLUMNS}
        rows={tableRows}
        total={count as number}
        page={page}
        pageSize={pageSize}
        searchPlaceholder="Kandidat, Stelle oder Unternehmen…"
        emptyTitle="Noch keine Vorschläge"
        emptyDescription="Neue Top-Matches werden im Radar erkannt und lassen sich dort oder hier als Vorschlag übernehmen."
        toolbar={
          <FilterSelect
            param="status"
            placeholder="Alle Status"
            options={Object.entries(PROPOSAL_STATUS).map(([value, def]) => ({
              value,
              label: def.label,
            }))}
          />
        }
      />
    </>
  );
}
