...
/**
 * A4-/E-Mail-Dokument aus Feldwerten + optionalen Positionen. Randloses,
 * professionelles Layout: vollflächiger grüner Kopf (kein weißer Rand oben),
 * ruhiger weißer Textkörper mit zwei dezenten Marken-Formen, dezente Fußzeile.
 * Festes „Papier"-Layout (theme-unabhängig) und druckecht.
 */
export function DokumentBlatt({
  ...
  return (
    ...
    >
      {/* Vollflächiger Marken-Kopf (edge-to-edge, kein weißer Rand oben) */}
      <div
        className="flex flex-wrap items-start justify-between gap-4 px-8 py-7 sm:px-12"
      ...
      >
        <div>
          ...
          {absAdresse && (
            <p className="mt-1 text-xs leading-relaxed whitespace-pre-line text-white/70">
              {absAdresse}
            </p>
          )}
          {w("absenderKontakt") && (
            <p className="mt-0.5 text-xs text-white/70">{w("absenderKontakt")}</p>
          )}
        </div>
      ...
      {/* dünner gelber Akzent unter dem Kopf */}
      <div className="h-1 w-full" style={{ backgroundColor: GELB }} />

      {/* Textkörper */}
      <div className="relative px-8 py-10 sm:px-12">
        {/* zwei dezente Marken-Formen */}
        <div
          aria-hidden
          className="pointer-events-none absolute -right-16 top-6 size-56 rounded-full"
          style={{ backgroundColor: GRUEN, opacity: 0.06 }}
        />
        <div
          aria-hidden
          className="pointer-events-none absolute -left-20 -bottom-4 size-52 rounded-full"
          style={{ backgroundColor: GELB, opacity: 0.08 }}
        />
      ...
      {/* Fußzeile (edge-to-edge) */}
      {w("absenderFuss") && (
        <div className="bg-neutral-50 px-8 py-4 text-xs text-neutral-500 sm:px-12">
          {w("absenderFuss")}
        </div>
      )}
    </div>
